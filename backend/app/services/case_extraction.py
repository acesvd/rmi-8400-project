...
CLAIM_PATTERNS = [r"claim\s*(?:#|number|no\.?):?\s*([A-Z0-9\-]+)"]
AUTH_PATTERNS = [r"(?:auth(?:orization)?|preauth)\s*(?:#|number|no\.?):?\s*([A-Z0-9\-]+)"]
MEMBER_PATTERNS = [r"member\s*(?:id|#|number|no\.?):?\s*([A-Z0-9\-]+)"]
...
def _normalized_case_json(raw: dict[str, Any], fallback: dict[str, Any]) -> dict[str, Any]:
    ...
    out["identifiers"] = {
        "claim_number": str(ids_raw.get("claim_number") or ids_fallback.get("claim_number") or "unknown"),
        "auth_number": str(ids_raw.get("auth_number") or ids_fallback.get("auth_number") or "unknown"),
        "member_id": str(ids_raw.get("member_id") or ids_fallback.get("member_id") or "unknown"),
    }

    reasons = raw.get("denial_reasons") if isinstance(raw.get("denial_reasons"), list) else []
    out["denial_reasons"] = [r for r in reasons if isinstance(r, dict)] or fallback.get("denial_reasons", [])
    ...
    out["submission_channels"] = _unique([str(c) for c in sub_channels]) or fallback.get("submission_channels", [])

    req_docs = raw.get("requested_documents") if isinstance(raw.get("requested_documents"), list) else []
    out["requested_documents"] = _unique([str(d) for d in req_docs]) or fallback.get("requested_documents", [])

    warnings = raw.get("warnings") if isinstance(raw.get("warnings"), list) else []
    out["warnings"] = _unique([str(w) for w in warnings])

    return out


def build_case_json(conn, case_id: str) -> tuple[dict[str, Any], list[str]]:
    rule_case_json, rule_warnings = build_case_json_rule_based(conn, case_id)

    pages = _collect_doc_text(conn, case_id)
    if not pages:
        return rule_case_json, _unique(rule_warnings + ["No documents uploaded."])

    client = OllamaClient()
    if not client.is_available():
        return rule_case_json, _unique(rule_warnings + ["Ollama unavailable. Using rule-based extraction."])

    try:
        llm_raw = _llm_extract_case_json(client, pages)
        merged = _normalized_case_json(llm_raw, rule_case_json)
        warnings = _unique((merged.get("warnings") or []) + rule_warnings)
        merged["warnings"] = warnings
        return merged, warnings
    except Exception as exc:
        return rule_case_json, _unique(rule_warnings + [f"Ollama extraction failed; fallback used: {exc}"])


def save_case_extraction(conn, *, case_id: str, case_json: dict[str, Any], warnings: list[str]) -> dict[str, Any]:
    extraction_id = new_id("ext")
    created_at = utc_now_iso()
    conn.execute(
        """
        INSERT INTO case_extractions (extraction_id, case_id, case_json, warnings, created_at)
        VALUES (?, ?, ?, ?, ?)
        """,
        (
            extraction_id,
...
def latest_case_extraction(conn, case_id: str) -> dict[str, Any] | None:
    row = conn.execute(
        """
        SELECT extraction_id, case_id, case_json, warnings, created_at
